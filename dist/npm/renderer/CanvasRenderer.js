...
var _paintOrder = require("../parsing/paintOrder");
...
function () {
  ...
  _createClass(CanvasRenderer, [{
    ...
    key: "renderTextNode",
    value: function renderTextNode(textBounds, color, font, textDecoration, textShadows, textStroke, paintOrder) {
      var _this4 = this;

      this.ctx.font = [font.fontStyle, font.fontVariant, font.fontWeight, font.fontSize, font.fontFamily].join(' ');
      textBounds.forEach(function (text) {
        paintOrder.forEach(function (layer) {
          switch (layer) {
            case _paintOrder.PAINT_LAYER.FILL:
            ...
            case _paintOrder.PAINT_LAYER.STROKE:
              if (textStroke && text.text.trim().length) {
                _this4.ctx.strokeStyle = textStroke.color.toString();
                _this4.ctx.lineWidth = textStroke.size;

                _this4.ctx.strokeText(text.text, text.bounds.left, text.bounds.top + text.bounds.height);
              } else {
                _this4.ctx.strokeStyle = '';
